# Hyperframes Composition Brief: Mise (v6 — the polish pass)
...
- Rendered video: `brag-output/brag.mp4` (the render, mastered by `brag-output/master.sh`)
...
- The food is modelled illustration, not flat fill: one radial body gradient per piece, a
  weighted underside, one soft specular from a gradient that reaches zero (never a stack
  of ellipses — that reads as a bullseye), a contact shadow, and mottle, sesame, wood grain
  and rice grains **generated by a seeded hash in the patch script and baked into the
  markup**, so there is texture without a random number or a filter at render time. Two
  shared `<g>` definitions (`#artThigh`, `#artGlaze`) are `<use>`d by all eight pieces of
  chicken, so they stay consistent and the file stays small.
...
3. **11.459** DEVICE — the customization in three screens beside the line "It starts by
   asking.": the headcount question; then all fourteen `RESTRICTIONS`, all thirteen
   `EQUIPMENT` tiles and both the heat and adventure scales cascading in (13.642); then
   the recap, "Your kitchen, set up" (15.825).
...
- Treatment: 0.52 under the dark open → 0.90 through the app beats → **0.62 for the whole
  cooking block so the contact sounds carry** → 0.86 for the plate → fade out
- Delivery level: the render itself lands at −17.8 LUFS, so `master.sh` adds +4.0 dB and a
  true-peak limiter at −2.2 dBFS to reach **−14.0 LUFS / −1.3 dBTP / LRA 7.9**. The
  composition's own gains cannot go higher without the bed plus an impact clipping, so the
  last 4 dB is a mastering step, not a mix change. Re-render → re-run `master.sh`.
...
- No SVG `<filter>` on the food. Form comes from gradients and stacked geometry; only the
  steam, the gloss sweep and the contact shadows use a filter, and those are CSS `blur`
  and `drop-shadow` on small boxes.
