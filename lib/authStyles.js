...
/* Same directional daylight as the app: warm high-left, cool low-right, very
   ...
   Exported, and imported by components/MiseApp.jsx for the in-app .surface.
   It used to be duplicated there at roughly two-thirds these alphas, and the
   copy then got dimmed again while trying to make the marble read more
   clearly — which did the opposite of what was wanted, because these
   gradients ARE the blue and the sunlight. Draining them made the marble more
   literally visible and the room darker. One definition now, so the app and
   the sign-in page can't drift apart again. */
...
/* Hint strength, for the in-app marble surface where the stone should be the
   thing you see and the light should only tint it.

   NOT simply the DAYLIGHT stack at lower opacity — that was tried and it
   doesn't work. Those four colours are near-white by design, which is correct
   over oak where they're most of what you see, but over pale marble at low
   alpha they cancel out: at 18% the strongest one shifted the composite by
   1.6 points of blue-red spread, i.e. invisible. Near-white over near-white
   is nothing.

   So these are properly saturated tints held at 9-12% instead. Same four
   lights, same positions, same story — cool north light high-left, warm bounce
   right, cool floor bounce low — but the colour is carried by saturation
   rather than by coverage. At 12% peak the marble is ~88-90% of what you see
   by compositing weight, and the cool-to-warm swing across the screen is
   about +7 to -12 in blue-red spread, which is a visible cast rather than a
   theoretical one.

   Measured, because this took several passes: marble.webp is 79% luminance
   (RGB 203,201,200, stddev 5.2) — pale, nearly flat, and still greyer than
   the #FAF5F4 paper. It never looked dark because something covered it; it
   looked dark because grey-201 beside paper-246 reads grey. Brightness is
   what fixes that, on the .surface rule in MiseApp's CSS. These alphas only
   decide how much colour rides along. */
export const DAYLIGHT_SOFT = [
  // Cool lights eased to .10/.08 now that the stone sits level with the paper:
  // at that brightness the blue is what still reads cold, and cold reads dark.
  // The warm bounce is left at .10 — warmth is what makes it look sunlit.
  "radial-gradient(60% 48% at 4% -4%, rgba(150,190,235,.10), transparent 62%)",
  "radial-gradient(70% 40% at 58% -6%, rgba(170,205,240,.08), transparent 64%)",
  "radial-gradient(44% 32% at 100% 46%, rgba(240,198,145,.10), transparent 62%)",
  "radial-gradient(76% 46% at 78% 106%, rgba(160,182,215,.08), transparent 66%)",
].join(",");
