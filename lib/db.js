...
function readConnectionString() {
  ...
  if (!v || !v.trim()) return { error: "DATABASE_URL is not set" };
  ...
  return { value: v, repaired: v !== before.trim() || before !== before.trim() };
}
...
if (conn.error) {
  ...
  try {
    ...
    console.log(`DATABASE: ${u.hostname}:${u.port || 5432}${u.pathname}` +
      (conn.repaired ? "  (cleaned up stray quotes/whitespace in DATABASE_URL)" : ""));
  } catch (_) {
