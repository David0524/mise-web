...
//
// On 3.8 Flash for "main" (released 2026-09-02). Confirmed free-tier on the
// Standard endpoint against ai.google.dev/gemini-api/docs/pricing on
// 2026-09-03: input, output and context caching all "Free of charge" on the
// Free Tier. Batch and Flex are paid-only on every Flash model; Standard and
// Priority are the free ones, and this file calls Standard.
//
// HOW TO REVERT, IF THIS GOES WRONG: change main back to "gemini-3.6-flash".
// Nothing else depends on the choice. Worth knowing because of what happened
// with 3.7 —
//
// 3.7 Flash was adopted and then reverted on the reasoning that it was "free
// in AI Studio but priced from day one through the standard API," so a key
// with billing enabled worked and one without it didn't. That reasoning does
// NOT hold up: the pricing page shows 3.7 Flash as "Free of charge" on the
// Free Tier for Standard, exactly like 3.6 and 3.8. Either the original
// diagnosis was wrong, or Google changed 3.7's status afterward — no way to
// tell from the docs. So the intermittent 502s on 3.7 have NO CONFIRMED
// CAUSE. Don't reason from that episode; just know the revert is one string.
const MODELS = { main: "gemini-3.8-flash", fast: "gemini-3.5-flash-lite" };
