...
const MODELS = { main: "gemini-3.7-flash", fast: "gemini-3.5-flash-lite" };
...
async function attempt(model, systemText, contents, generationConfig, apiKey) {
  ...
  // timeout instead. 25s gives a slow-but-real response room to land while
  // still leaving budget for retries/rotation within the function's overall
  // time limit.
  ...
  try {
    ...
    if (e.name === "AbortError") {
      ...
      err.status = 504;
      err.detail = "request timed out after 25000ms with no response";
      throw err;
    }
    throw e;
  } finally {
