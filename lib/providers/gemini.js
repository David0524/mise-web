...
/* Time budget for the whole call, all keys and retries included.
   ...
   ATTEMPT_MS is how long any single request gets; BUDGET_MS caps the total.
   Sized from a real production log (2026-09-03 14:56) rather than guessed:

     14:56:13  chat in
     14:56:43  key #1 timed out with no response   <- 30s, full ATTEMPT_MS
     14:56:58  key #2 timed out with no response   <- 15s, the leftover budget
     14:56:58  budget spent, keys #3 and #4 never tried
               chat fail after 45008ms

   Two lessons in that. First, Gemini HANGS on this call rather than erroring
   — no 429, no 503, just silence — so the failure is latency, not quota.
   Second, splitting the budget across keys made it worse: key #2 got 15
   seconds it could never succeed in, and two keys were never reached. When a
   request hangs, a different key doesn't help; it's the same model doing the
   same slow work.

   So one attempt now gets nearly all of it. 45s for the request, 50s total,
   which still leaves 10s under the route's maxDuration of 60 for auth, body
   parsing and serialization. The 100s runaway that originally motivated a
   flat cap is still prevented, because the total is still bounded. */
...
export async function callModel(messages, systemText, { tier = "main", maxTokens = 1000, deadlineAt } = {}) {
  ...
  /* Whichever runs out first: this provider's own budget, or the caller's
     deadline for the whole request. The caller's is the one that knows about
     cold-start boot and the auth queries that already happened, so on a cold
     instance it will be the binding one — which is the point. Never later
     than BUDGET_MS, so this can only be more conservative, never less. */
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    /* ONE attempt per key on a 503, not two.
       From a real failure (2026-09-21 21:06): key #1 returned 503 after 13.3s,
       was retried on the same key, returned 503 again at 36.6s — and those two
       attempts had consumed 36 of the 50-second budget. Key #2 got the 13
       seconds that were left and keys #3 and #4 were never reached at all.

       A 503 means Gemini's capacity for that model is saturated, and it takes
       13-23 SECONDS to say so. Asking the same key again is the most expensive
       possible response: it is the slowest kind of failure, and the condition
       it is waiting on is the one thing a short pause cannot change. The other
       keys are separate projects, so they are the only thing in this loop with
       a real chance of landing. Spend the budget on breadth, not repetition. */
    const attemptsOnThisKey = 1;
    for (let attemptNum = 0; attemptNum < attemptsOnThisKey; attemptNum++) {
      ...
      try {
        ...
        break;   // one attempt per key; the outer loop decides whether to rotate
      }
    }

    /* Only rotate to the next key for a rate limit (key-specific quota, a
       different key genuinely has its own) or if 503 persisted through all
       retries on this one. Anything else — a malformed request, a content
       block, a bad model name — would fail identically no matter which key
       sends it, so trying the rest would waste them for nothing.

       504 (our own synthesized hang) USED to rotate, on the theory that a
       stuck connection is more likely to still be stuck than an overloaded
       server is. The 2026-09-03 log disproved it: key #1 hung for the full
       30s, key #2 hung for the full 15s that were left, and keys #3 and #4
       were never reached. A hang here is the model being slow on a heavy
       request, which is identical on every key. Rotating just spent the
       budget that the first attempt should have had, so a hang now fails
       fast and gives its whole allowance to one honest try. */
    if (lastErr.status !== 429 && lastErr.status !== 503) {
      /* Report the timeout ACTUALLY used, not the configured maximum. This line
         said "after 45s" for an attempt that was cut off at 13s because the
         budget was nearly spent — which made a budget-exhaustion look like a
         hung connection, and pointed diagnosis in the wrong direction. */
      const label = lastErr.status === 504
        ? `${lastErr.detail || "timed out"} — not a key problem, not rotating`
        : `${lastErr.status || "(no status)"} ${lastErr.detail || lastErr.blocked || lastErr.message}`;
      console.error("Gemini giving up:", label);
      throw new Error("upstream");
    }
    const why = lastErr.status === 429 ? "rate-limited" : "overloaded (503)";
    console.error(`Gemini key #${idx + 1} of ${keys.length} ${why}, moving to next key`);

    /* Two keys down with 503s means the capacity problem is the MODEL, not the
       keys — every key points at the same saturated pool. The lighter model is
       a separate pool and is usually the one with headroom, so the remaining
       keys try that instead of repeating a request already known to bounce.

       A recipe from flash-lite is a small step down in quality. No recipe at
       all, after fifty seconds of waiting, is the actual failure being
       avoided. Only fires on 503 — a 429 is a quota problem, and a different
       model on the same project would hit the same quota. */
    if (lastErr.status === 503 && idx >= 1 && model !== MODELS.fast) {
      console.error(`Gemini: ${model} saturated across ${idx + 1} keys — dropping to ${MODELS.fast} for the rest`);
      model = MODELS.fast;
    }
