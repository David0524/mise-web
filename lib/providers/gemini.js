...
//
// On 3.6 Flash for "main". History, and the correction to it:
//
// This was reverted from 3.7 Flash on the reasoning that 3.7 was "free in AI
// Studio but priced from day one through the standard API," which would have
// meant a key with billing enabled worked and a key without it didn't. That
// reasoning does NOT hold up: checked against ai.google.dev/gemini-api/docs/pricing
// on 2026-09-02, Gemini 3.7 Flash shows "Free of charge" on the Free Tier for
// the Standard endpoint, exactly like 3.6 and 3.8. Either the original
// diagnosis was wrong, or Google changed 3.7's free-tier status afterward —
// no way to tell which from the docs alone.
//
// So the intermittent 502s on 3.7 have NO CONFIRMED CAUSE. Treat the revert
// as an unexplained workaround, not a solved problem. Don't reason from it.
//
// 3.8 Flash (released 2026-09-02) is also free-tier on Standard. Not adopted:
// the observed failures here were 504s from this file's own timeout, i.e. a
// latency problem, which a model swap doesn't address — and changing models
// on launch day is what produced the last round of confusion. Batch and Flex
// are paid-only on every Flash model; only Standard and Priority are free.
