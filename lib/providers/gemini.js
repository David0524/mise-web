...
const MODELS = { main: "gemini-3.8-flash", fast: "gemini-3.8-flash-lite" };
...
export async function callModel(messages, systemText, { tier = "main", maxTokens = 1000 } = {}) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    const attemptsOnThisKey = 3;
    for (let attemptNum = 0; attemptNum < attemptsOnThisKey; attemptNum++) {
      if (attemptNum > 0) await new Promise((r) => setTimeout(r, 700));
      try {
    ...
    // block — would fail identically no matter which key sends it, so
    // trying the rest of the keys on those would just waste them for
    // nothing.
