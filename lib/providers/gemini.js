...
export async function callModel(messages, systemText, { tier = "main", maxTokens = 1000 } = {}) {
  ...
  try {
    ...
    console.error("Gemini ok via thinkingLevel:LOW");
    return text;
  } catch (e) {
    ...
    console.error("Gemini rejected thinkingLevel shape (400), falling back:", e.detail);

    try {
      const text = await attempt(model, systemText, contents, {
        maxOutputTokens: Math.max(maxTokens, 2000),
      });
